...
{
	if (data == nullptr || rhs.data == nullptr) return false;
	if (size != rhs.size) return false;
	if (capacity != rhs.capacity) return false;

	for (iterator it = begin(), iterator it = lhs.begin(), iterator it2 = rhs.begin(); it != rhs.end(); ++it, ++it2)
	{
...
{
	if (lhs.data == nullptr || rhs.data == nullptr) return false;
	if (lhs.size != rhs.size) return false;
	if (lhs.capacity != rhs.capacity) return false;

	for (iterator it = lhs.begin(), iterator it2 = rhs.begin(); it != lhs.end(); ++it, ++it2)
	{
		if (*it != *it2) return false;
	}
	return true;
...
std::string Array<T>::ToString() const
{
	...
	if (data == nullptr) dataString = "null";
	else
	{
		for (iterator it = begin(); it != end(); ++it) dataString += std::to_string(*it) + ",";
	}
	return dataString + "| Size: " + std::to_string(size) + "| Capacity: " + std::to_string(capacity);
}